...
ideaController.getIdeas = (req, res, next) => {
  const queryText =
    'SELECT Ideas.*, array_agg(tech_stacks.name) AS techStacks FROM Ideas JOIN Idea_tech_stacks ON Idea_tech_stacks.idea_id = Ideas.idea_id JOIN tech_stacks ON tech_stacks.tech_id=Idea_tech_stacks.tech_id GROUP BY Ideas.idea_id';
